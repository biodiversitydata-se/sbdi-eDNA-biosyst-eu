...
::: {.callout-note}
`eventDate` is stored as a `start/end` interval string, so we parse the start date explicitly rather than passing it straight to `month()`/`yday()`. A few samples (e.g. in the lysate dataset) have an incomplete date — just a bare year, with no month or day. Rather than guess (e.g. assigning them Jan 1st), `parse_date_time()` leaves those as `NA`, which excludes them downstream instead of introducing a misleading date.
:::
...
`100000` is the threshold used in earlier IBA analyses of this data — check the `summary()` output above and adjust it if it excludes far more or fewer samples than expected for your dataset.

The two datasets differ hugely in size (homogenate: ~860 samples; lysate: ~4,550), so combining them as-is would let lysate dominate the analysis purely by sample count, and would make PCoA's eigendecomposition slow (O(n³) in the number of samples). We downsample lysate down to homogenate's size instead, keeping the samples balanced 50/50:
...
Many samples share the same, or a nearby, physical site (repeated visits to the same trap over the season), so plotting raw coordinates hides most of them behind one another. The small amount of random jitter above spreads samples from the same site into a visible cluster instead of a single point.
...
[![](figures/3_map.png)](figures/3_map.png){target="_blank"}
