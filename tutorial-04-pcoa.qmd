...
This alone reduces the row count considerably (e.g. ~7-fold for the homogenate dataset), but some clusters can still be entirely zero within just the samples selected by `ix` (e.g. clusters only present in other samples). Left in, those constant rows add a block of tied ranks to the Spearman correlation below, which can shift the ranking — and therefore the correlation — of the real values. `ix` is the balanced, pooled sample set from [step 3](tutorial-03-map.qmd):
